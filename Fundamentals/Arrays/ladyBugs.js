function ladyBugs(input) {
    let fieldLength = Number(input.shift());
    let bugPosition = input.shift().split(' ');
    let field = Array(fieldLength).fill(0);

    for (let i of bugPosition) {
        if (field[i] !== undefined) field[i] = 1;
    }
    while (input.length > 0) {
        let [currPosition, direction, jump] = input.shift().split(' ');
        if (field[currPosition] !== 1 || jump == 0) {
            continue;
        }
        if (jump < 0) {
        ...
        jump = Math.abs(jump);
        while (true) {
            if (direction === 'left') newPosition = currPosition - jump;
            else newPosition = currPosition + jump;
            if (field[newPosition] !== undefined) {
